...
rm(train_index, num_long_trips_removed, num_old_age_removed, smp_size)
```
# Linear Model
...
```{r gbm model fitting, cache = TRUE}
library('gbm')
...
```{r}
y_test<-train_data$tripduration
predicted = predict(xgb_model,train_data)
residuals = y_test - predicted
RMSE = sqrt(mean(residuals^2))
cat('The root mean square error of the train data is ', round(RMSE,3),'\n')
```
...
# Plot predictions vs test data
ggplot(my_data,aes(predicted, observed)) + geom_point(color = "darkred", alpha = 0.5) + 
    geom_smooth(method=lm)+ ggtitle('Gradient Boosted Machines ') + ggtitle("Extreme Gradient Boosting: Prediction vs Test Data") +
      xlab("Predecited Trip Duration in Minutes ") + ylab("Observed Trip Duration in Minutes") + 
        theme(plot.title = element_text(size=16,hjust = 0.5),
         axis.text.y = element_text(size=12), axis.text.x = element_text(size=12,hjust=.5),
         axis.title.x = element_text(size=14), axis.title.y = element_text(size=14))
...
```{r, warning =F}
trellis.par.set(caretTheme())
plot(rfmodel, metric = "RMSE", plotType = "level",
     scales = list(x = list(rot = 90)))
```
